    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = BusinessAdapter(
            ...
            onClick = { business: Business ->
                val subtitle = business.categories
                    .mapNotNull { it.name?.trim()?.takeIf(String::isNotEmpty) }
                    .joinToString(separator = " · ")

                val intent = Intent(this, BusinessMenuActivity::class.java).apply {
                    putExtra(BusinessMenuActivity.EXTRA_BUSINESS_ID, business.id)
                    putExtra(BusinessMenuActivity.EXTRA_BUSINESS_NAME, business.name)
                    putExtra(BusinessMenuActivity.EXTRA_BUSINESS_SUBTITLE, subtitle)
                    putExtra(BusinessMenuActivity.EXTRA_BUSINESS_LOGO, business.logo)
                    putExtra(BusinessMenuActivity.EXTRA_BUSINESS_RATING, business.rating)
                    putExtra(BusinessMenuActivity.EXTRA_BUSINESS_REVIEWS, business.products.size)
                }
